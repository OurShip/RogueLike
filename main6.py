...
def place_objects(room):
    ...
    for i in range(num_monsters):
        ...
        if not is_blocked(x, y):
            if libtcod.random_get_int(0, 0, 100) < 80: # chance of getting an orc
                # create an orc
                fighter_component = Fighter(hp=10, defense=0, power=5, death_function=monster_death)
...
def player_death(player):
    ...
    #for added effect, transform the player in to a corpse!
    player.char = '%'
    player.color = libtcod.dark_red

def monster_death(monster):
    # transform it in to a nasty corpse! it doesn't block, can't be
    # attacked and it does not move
...
# the list of objects w/ just the player
objects = [player]
...
message('Welcome stranger! Prepare to be killed!', libtcod.red)
